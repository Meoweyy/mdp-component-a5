...
int parse_android_map_json(const char* json_string, SharedAppContext* context) {
    ...
    // Defaults are the CENTRE cell of the start box (see note below).
    int robot_x_val = 1; // Default centre cell (0-indexed) -> 15cm
    int robot_y_val = 1; // Default centre cell (0-indexed) -> 15cm
    ...
    // No conversion: the com.mdp.controller tablet app already reports the CENTRE
    // cell of the 3x3 footprint (see RobotPose.kt), which is what the algo server
    // wants. Do NOT re-add the +1 here.
    //
    // WARNING: the older com.example.sc2079 app instead sends the BOTTOM-LEFT cell
    // (GridMapClass.java, sendArenaDataBluetooth). If you switch back to that app,
    // restore "+ 1" on both lines below or the start pose will be one cell short.
    context->robot_start_x = robot_x_val;
    context->robot_start_y = robot_y_val;
